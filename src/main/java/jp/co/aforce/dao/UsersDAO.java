...
public class UsersDAO extends DAO {
	public Users search(String id, String pw) throws Exception {
		...
		try (Connection con = getConnection();
				PreparedStatement ps = con.prepareStatement(
						"SELECT MEMBER_ID, PASSWORD, MANAGER, LAST_NAME, deleted FROM users WHERE MEMBER_ID=? And PASSWORD=?");) {
			ps.setString(1, id);
			ps.setString(2, pw);
			try (ResultSet rs = ps.executeQuery();) {
				if (rs.next()) {
					...
					loginUser.setDeleted(rs.getInt("deleted"));
	...
	public DuplicateResult check(String id, String mail) throws Exception {
		...
		try (Connection con = getConnection();
				PreparedStatement ps = con.prepareStatement(
						"SELECT MEMBER_ID, MAIL_ADDRESS, deleted FROM users WHERE MEMBER_ID = ? OR MAIL_ADDRESS = ?");) {
			ps.setString(1, id);
			ps.setString(2, mail);
			try (ResultSet rs = ps.executeQuery();) {
				while (rs.next()) {
	...
	public int registration(Users setUser) throws Exception {
		...
		try (Connection con = getConnection();
				PreparedStatement ps = con.prepareStatement(
						"INSERT INTO users(MEMBER_ID,PASSWORD,LAST_NAME,FIRST_NAME,ADDRESS,MAIL_ADDRESS,MANAGER,deleted) VALUES (?, ?, ?, ?, ?, ?, 0, 0)");) {
			ps.setString(1, id);
			ps.setString(2, pw);
	...
	public int delete(String id) throws Exception {
	    ...
	    try (Connection con = getConnection();
	         PreparedStatement ps = con.prepareStatement(
	             "UPDATE users SET deleted = 1, MEMBER_ID = CONCAT(MEMBER_ID, '_削除'),MAIL_ADDRESS = CONCAT(MAIL_ADDRESS, '_削除') WHERE MEMBER_ID = ?")) {
