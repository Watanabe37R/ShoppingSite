			...
			<form								
				...
				<div class="top-area">							
					...
					<div class="summary-grid">						
						<div class="summary-item">					
							<label>商品ID</label>				
							<input type="text" id="productId" name="productId" value="${mode == 'register' ? '' : products.productId}" ${mode == 'register' ? '' : 'readonly'}>				
							<span id="productIdM"></span>				
						</div>					
						<div class="summary-item">					
							<label>商品名</label>				
							<input type="text" id="productName" name="productName" value="${mode == 'register' ? '' : products.productName}" ${mode == 'view' || mode == 'delete' ? 'readonly' : ''}>				
							<span id="productNameM"></span>				
						</div>					
						<div class="summary-item">					
							<label>商品価格</label>				
							<input type="text" id="price" name="price" value="${mode == 'register' ? '' : products.price}" ${mode == 'view' || mode == 'delete' ? 'readonly' : ''}>				
							<span id="priceM"></span>				
						</div>					
					</div>						
				</div>							
				...
				<div class="detail-area">							
					<div class="detail-grid">						
							...
							<c:choose>
								<c:when test="${mode == 'register' || mode == 'edit'}">
									<div class="reference-group">
										<button type="button" onclick="openMaker()">メーカーＩＤ</button>
										<input type="text" id="makerId" name="makerId" value="${mode == 'register' ? '' : products.makerId}" ondblclick="openMaker()" readonly>		
									</div>
								</c:when>
								<c:otherwise>
									<input type="text" name="makerId" value="${products.makerId}" readonly>	
								</c:otherwise>	
							</c:choose>	
						</div>					
						<div class="detail-item">					
							<label>メーカー名</label>
							<input type="text" id="makerName" name="makerName" value="${mode == 'register' ? '' : products.makerName}" readonly>
						</div>					
						<div class="detail-item">					
							<label>税率区分</label>				
							<c:choose>				
						...
						<div class="detail-item">					
							<label>税</label>				
							<input type="text" id="tax" name="tax" value="${mode == 'register' ? '' : products.tax}" ${mode == 'view' || mode == 'delete' ? 'readonly' : ''}>				
							<span id="taxM"></span>				
						</div>					
						<div class="detail-item">					
							<label>カテゴリID</label>
							...
							<c:when test="${mode == 'register' || mode == 'edit'}">
								<div class="reference-group">
									<button type="button" onclick="openCategory()">カテゴリID</button>
									<input type="text" id="categoryId" name="categoryId" value="${mode == 'register' ? '' : products.categoryId}" ondblclick="openCategory()" readonly>
								</div>
							</c:when>
							<c:otherwise>
								<input type="text" id="categoryId" name="categoryId" value="${products.categoryId}" readonly>
							</c:otherwise>
							</c:choose>
						</div>
						<div class="detail-item">					
							<label>カテゴリ名</label>				
							<input type="text" id="categoryName" name="categoryName" value="${mode == 'register' ? '' : products.categoryName}" readonly>				
						</div>					
						<div class="detail-item">					
							<label>販売区分</label>				
							<c:choose>				
						...
						<div class="detail-item">					
							<label>在庫数</label>				
							<input type="text" id="stock" name="stock" value="${mode == 'register' ? '' : products.stock}" ${mode == 'view' || mode == 'delete' ? 'readonly' : ''}>				
							<span id="stockM"></span>				
						</div>					
						<div class="detail-item full">					
							<label>摘要</label>				
							<textarea id="abstractText" name="abstractText" ${mode == 'view' || mode == 'delete' ? 'readonly' : ''}>${mode == 'register' ? '' : products.abstractText}</textarea>				
							<span id="abstractTextM"></span>				
						</div>					
					</div>						
					<!-- 閲覧の時はそもそもボタンを出さない -->						
					<c:if test="${mode!='view'}">						
...
<script src="${pageContext.request.contextPath}/js/openOtherWindow.js"></script>
<script>
	const contextPath = "${pageContext.request.contextPath}";
	const mode = "${mode}";
</script>
<script src="${pageContext.request.contextPath}/js/productValidation.js"></script>
<script>
